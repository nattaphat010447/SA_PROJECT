...
export const swipeUser = async (req, res) => {
    try {
        ...
        const checkSwipe = await pool.query('SELECT * FROM swipes WHERE requester_id = $1 AND target_id = $2', [myId, targetId]);
        if (checkSwipe.rows.length > 0) {
            return res.status(400).json({ message: 'You already swiped this user' });
        }
        // 1. บันทึกการปัดลงตาราง swipes
        await pool.query('INSERT INTO swipes (requester_id, target_id, status) VALUES ($1, $2, $3)', [myId, targetId, status]);
        let isMatch = false;
        let matchId;
        // 2. ถ้าเรากด LIKE ให้ไปเช็คว่าเขากด LIKE เราไว้ก่อนหน้านี้ไหม
        if (status === 'LIKE') {
            const checkMatch = await pool.query('SELECT * FROM swipes WHERE requester_id = $1 AND target_id = $2 AND status = $3', [targetId, myId, 'LIKE']);
            // ถ้าเจอกด LIKE เหมือนกัน -> เกิดการ MATCH!
            if (checkMatch.rows.length > 0) {
                isMatch = true;
                // บันทึกลงตาราง matches
                const matchRes = await pool.query('INSERT INTO matches (user_one_id, user_two_id) VALUES ($1, $2) RETURNING id', [myId, targetId] // หรือ [targetId, myId] ก็ได้
                );
...
export const unmatchUser = async (req, res) => {
    try {
        ...
        const checkMatch = await pool.query('SELECT * FROM matches WHERE id = $1 AND (user_one_id = $2 OR user_two_id = $2)', [matchId, myId]);
        if (checkMatch.rows.length === 0) {
            return res.status(403).json({ message: 'Access denied: Cannot unmatch' });
        }
        // 3. เปลี่ยนสถานะ is_active ให้เป็น false (Soft Delete)
        await pool.query('UPDATE matches SET is_active = false WHERE id = $1', [matchId]);
